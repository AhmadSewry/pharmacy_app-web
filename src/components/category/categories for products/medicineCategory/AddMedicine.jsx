...
const categoryIdFromState = location.state?.id || 0;
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`http://localhost:5200/api/MedicineCategory/${categoryIdFromState}`);
  ...
  const handleSubmit = async () => {
    ...
    if (!hasError) {
      try {
        ...
        formData.append("CategoryId", newProduct.categoryId); // ثابت
  ...
  <DialogContent dividers>
    ...
    >
      ...
      <TextField
        ...
        helperText={errors.sellPrice || "أدخل سعر البيع بالعملة المناسبة"}
        fullWidth
      />
